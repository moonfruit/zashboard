...
const props = defineProps<{
  menuItems: MenuItem[]
  activeMenuKey: LOCAL_MENU_KEY
}>()

const emit = defineEmits<{
  (e: 'menu-click', key: LOCAL_MENU_KEY): void
}>()
...
const getMenuItemAtPosition = (x: number): LOCAL_MENU_KEY | null => {
  ...
  for (const itemEl of menuItemRefs.value) {
    ...
    if (relativeX >= itemRelativeX && relativeX <= itemRelativeX + itemWidth) {
      return itemEl.dataset.key as LOCAL_MENU_KEY
    }
  }
